...
import style from './comprasTarjeta.module.css';
...
import { FaUser, FaTruck, FaRegSmile, FaCreditCard } from 'react-icons/fa';
...
const CompraFinalizadaTarjetas = React.memo(() => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            // Extraer y guardar información de la tarjeta si está disponible
            if (initialNewEvent && initialNewEvent.payMethod) {
                setCardInfo({
                    tipo: initialNewEvent.payMethod.paymentTypeId === 'credit_card' ? 'Crédito' : 'Débito',
                    ultimos4: initialNewEvent.payMethod.lastFourDigits || '****',
                    marca: initialNewEvent.formData?.payment_method_id || '',
                    titular: initialNewEvent.payMethod.cardholderName || '',
                    cuotas: initialNewEvent.formData?.installments || 1
                });
            }
    ...
    const renderSection = () => {
        if (activeSection === 'comprador') {
            return (
                ...
                >
                    ...
                    <motion.div className={style.detailContainer} variants={itemVariants}>
                        ...
                        <p className={`${style.detailValue} ${style.highlightValue}`}>Tarjeta {cardInfo?.marca?.toUpperCase() || ''} {cardInfo?.tipo} ****{cardInfo?.ultimos4}</p>
                    </motion.div>

                    {cardInfo && cardInfo.cuotas > 1 && (
                        <motion.div className={style.detailContainer} variants={itemVariants}>
                            <p className={style.detailLabel}>Cuotas:</p>
                            <p className={style.detailValue}>{cardInfo.cuotas} cuota(s)</p>
                        </motion.div>
                    )}
                </motion.div>
            );
        } else if (activeSection === 'producto') {
            // El renderizado de productos se mantiene igual
            return (
                <motion.div
                    key="producto"
                    initial="hidden"
            ...
            // El renderizado de la sección de envío se mantiene igual
            return (
                <motion.div
                    key="envio"
                    initial="hidden"
        ...
        } else if (activeSection === 'pago') {
            // Nueva sección para detalles del pago con tarjeta
            return (
                <motion.div
                    key="pago"
                    initial="hidden"
                    ...
                    className={style.section}
                >
                    <h2 className={style.sectionTitle}>Detalles del Pago</h2>

                    <div className={style.paymentCard}>
                        <div className={style.cardHeader}>
                            <FaCreditCard className={style.cardIcon} />
                            <span className={style.cardBrand}>{cardInfo?.marca?.toUpperCase() || 'TARJETA'}</span>
                        </div>
                        <div className={style.cardBody}>
                            <div className={style.cardField}>
                                <span className={style.cardFieldLabel}>Tipo:</span>
                                <span className={style.cardFieldValue}>{cardInfo?.tipo || 'No especificado'}</span>
                            </div>
                            <div className={style.cardField}>
                                <span className={style.cardFieldLabel}>Número:</span>
                                <span className={style.cardFieldValue}>**** **** **** {cardInfo?.ultimos4 || '****'}</span>
                            </div>
                            <div className={style.cardField}>
                                <span className={style.cardFieldLabel}>Titular:</span>
                                <span className={style.cardFieldValue}>{cardInfo?.titular || 'No especificado'}</span>
                            </div>
                            <div className={style.cardField}>
                                <span className={style.cardFieldLabel}>Cuotas:</span>
                                <span className={style.cardFieldValue}>{cardInfo?.cuotas || 1}</span>
                            </div>
                            <div className={style.cardField}>
                                <span className={style.cardFieldLabel}>Estado:</span>
                                <span className={`${style.cardFieldValue} ${style.approved}`}>Aprobado</span>
                            </div>
                            <div className={style.cardField}>
                                <span className={style.cardFieldLabel}>Monto Total:</span>
                                <span className={style.cardFieldValue}>${parseFloat(newEvent?.datosEnvio?.totalPrice).toLocaleString('es-AR')}</span>
                            </div>
                        </div>
                    </div>

                    <div className={style.paymentNote}>
                        <p>El pago ha sido procesado correctamente. Recibirás un comprobante en tu correo electrónico.</p>
                    </div>
                </motion.div>
            );
        }
            ...
            >
                ...
                >
                    ...
                    <Tooltip title="Productos comprados" arrow placement="top">
                        ...
                        >
                            <motion.div whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.95 }}>
                                <Badge badgeContent={newEvent?.products?.length} color="error" overlap="circular">
                    ...
                    {/* Nuevo botón para detalles de pago */}
                    <Tooltip title="Detalles del pago" arrow placement="top">
                        <IconButton
                            className={`${style.iconButton} ${activeSection === 'pago' ? style.active : ''}`}
...
CompraFinalizadaTarjetas.displayName = 'CompraFinalizadaTarjetas';
export default CompraFinalizadaTarjetas;